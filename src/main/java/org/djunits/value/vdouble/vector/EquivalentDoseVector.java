...

{
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Double> or SortedMap<Integer, EquivalentDose> */
     ...
     * @param data SortedMap&lt;Integer, Double&gt; or SortedMap&lt;Integer, EquivalentDose&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit EquivalentDoseUnit; the display unit of the vector data, and the unit of the data points when the data
     *            is expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public EquivalentDoseVector(final SortedMap<Integer, ? extends Number> data, final int size,
            final EquivalentDoseUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof EquivalentDose
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public EquivalentDoseVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
