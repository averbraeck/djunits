...

{
    ...
    /**
     * Construct a FloatLinearDensityVector from a float[] object. The Float values are expressed in the displayUnit, and will be
    ...
    /**
     * Construct a FloatLinearDensityVector from a float[] object. The Float values are expressed in the displayUnit. Assume that the
    ...
    /**
     * Construct a FloatLinearDensityVector from a float[] object with SI-unit values. Assume that the StorageType is DENSE since we
    ...
    /**
     * Construct a FloatLinearDensityVector from an array of FloatLinearDensity objects. The FloatLinearDensity values are each expressed in their own
     * unit, but will be internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data FloatLinearDensity[]; the data for the vector
     * @param displayUnit LinearDensityUnit; the display unit of the values when printing
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatLinearDensityVector(final FloatLinearDensity[] data, final LinearDensityUnit displayUnit, final StorageType storageType)
    {
    ...
    /**
     * Construct a FloatLinearDensityVector from a list of Number objects or a list of FloatLinearDensity objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatLinearDensity objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatLinearDensity objects, each FloatLinearDensity has its
     ...
     * @param data List&lt;Float&gt; or List&lt;LinearDensity&gt;; the data for the vector
     * @param displayUnit LinearDensityUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatLinearDensityVector(final List<? extends Number> data, final LinearDensityUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(new float[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof FloatLinearDensity ? FloatVectorData.instantiate(data, IdentityScale.SCALE, storageType)
    ...
    /**
     * Construct a FloatLinearDensityVector from a list of Number objects or a list of FloatLinearDensity objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatLinearDensity objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatLinearDensity> */

    /**
     * Construct a FloatLinearDensityVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatLinearDensity objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatLinearDensity objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatLinearDensity objects, each FloatLinearDensity has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatLinearDensity&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit LinearDensityUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatLinearDensityVector(final SortedMap<Integer, ? extends Number> data, final int size, final LinearDensityUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatLinearDensity
     ...
     * assume that they are expressed using SI units. When the data consists of FloatLinearDensity objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatLinearDensityVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatLinearDensityVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
