...
import org.djunits.old.value.vfloat.scalar.FloatRadioActivity;
...

{
    ...
    /**
     * Construct a FloatRadioActivityVector from a list of Number objects or a list of FloatRadioActivity objects. Note that the
     * displayUnit has a different meaning depending on whether the list contains Number objects (e.g., Float objects) or
     * FloatRadioActivity objects. In case the list contains Number objects, the displayUnit indicates the unit in which the
     ...
     * unit when printing.
     * @param data the data for the vector as a List&lt;Float&gt; or List&lt;RadioActivity&gt;
            ...
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(new float[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof FloatRadioActivity
    ...
    /**
     * Construct a FloatRadioActivityVector from a list of Number objects or a list of FloatRadioActivity objects. When data
    ...
    /* CONSTRUCTORS WITH Map<Integer, Float> or Map<Integer, FloatRadioActivity> */

    /**
     * Construct a FloatRadioActivityVector from a (sparse) map of index values to Number objects or a (sparse) map of index
     * values to of FloatRadioActivity objects. Using index values is particularly useful for sparse vectors. The size parameter
     * indicates the size of the vector, since the largest index does not have to be part of the map. Note that the displayUnit
     * has a different meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatRadioActivity
     ...
     * @param data the data for the vector as a Map&lt;Integer, Float&gt; or Map&lt;Integer, FloatRadioActivity&gt;
            ...
            final RadioActivityUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.values().iterator().next() instanceof FloatRadioActivity
     ...
     */
    public FloatRadioActivityVector(final Map<Integer, ? extends Number> data, final int size,
            final RadioActivityUnit displayUnit)
    {
