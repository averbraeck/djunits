...
public class ElectricalPotentialVector extends DoubleVectorRel<ElectricalPotentialUnit, ElectricalPotential, ElectricalPotentialVector>

{
    ...
    /**
     * Construct an ElectricalPotentialVector from a double[] object. The double values are expressed in the displayUnit, and will be printed
     * using the displayUnit.
     * @param data the data for the vector, expressed in the displayUnit
     * @param displayUnit the unit of the values in the data array, and display unit when printing
     * @param storageType the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public ElectricalPotentialVector(final double[] data, final ElectricalPotentialUnit displayUnit, final StorageType storageType)
    {
        this(DoubleVectorData.instantiate(data, displayUnit.getScale(), storageType), displayUnit);
    }

    /**
     * Construct an ElectricalPotentialVector from a double[] object. The double values are expressed in the displayUnit. Assume that the
    ...
    /**
     * Construct an ElectricalPotentialVector from a double[] object with SI-unit values. Assume that the StorageType is DENSE since we offer
    ...
    /**
     * Construct an ElectricalPotentialVector from an array of ElectricalPotential objects. The ElectricalPotential values are each expressed in their own unit, but will be
     * internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data the data for the vector
     * @param displayUnit the display unit of the values when printing
     * @param storageType the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public ElectricalPotentialVector(final ElectricalPotential[] data, final ElectricalPotentialUnit displayUnit, final StorageType storageType)
    {
    ...
    /**
     * Construct an ElectricalPotentialVector from a list of Number objects or a list of ElectricalPotential objects. Note that the displayUnit has a
     * different meaning depending on whether the list contains Number objects (e.g., Double objects) or ElectricalPotential objects. In case
     * the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed, as
     * well as the unit in which they will be printed. In case the list contains ElectricalPotential objects, each ElectricalPotential has its own unit, and
     ...
     * @param data the data for the vector as a List&lt;Double&gt; or List&lt;ElectricalPotential&gt;
     ...
     * @param storageType the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public ElectricalPotentialVector(final List<? extends Number> data, final ElectricalPotentialUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(new double[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof ElectricalPotential ? DoubleVectorData.instantiate(data, IdentityScale.SCALE, storageType)
    ...
    /**
     * Construct an ElectricalPotentialVector from a list of Number objects or a list of ElectricalPotential objects. When data contains numbers such as
     * Double, assume that they are expressed using SI units. When the data consists of ElectricalPotential objects, they each have their own
    ...
    /* CONSTRUCTORS WITH Map<Integer, Double> or Map<Integer, ElectricalPotential> */

    /**
     * Construct an ElectricalPotentialVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to of
     * ElectricalPotential objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size of the
     * vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different meaning
     * depending on whether the map contains Number objects (e.g., Double objects) or ElectricalPotential objects. In case the map contains
     * Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the unit in
     * which they will be printed. In case the map contains ElectricalPotential objects, each ElectricalPotential has its own unit, and the displayUnit is
     ...
     * @param data the data for the vector as a Map&lt;Integer, Double&gt; or Map&lt;Integer, ElectricalPotential&gt;
     * @param size the size off the vector, i.e., the highest index
     * @param displayUnit the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     * @param storageType the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public ElectricalPotentialVector(final Map<Integer, ? extends Number> data, final int size, final ElectricalPotentialUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.values().iterator().next() instanceof ElectricalPotential
     ...
     * that they are expressed using SI units. When the data consists of ElectricalPotential objects, they each have their own unit, but will
     ...
     * @param storageType the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public ElectricalPotentialVector(final Map<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size the size off the vector, i.e., the highest index
     */
    public ElectricalPotentialVector(final Map<Integer, ? extends Number> data, final int size)
    {
