...

{
     ...
     * @param displayUnit VolumeUnit; the unit of the values in the data array, and display unit when printing
    ...
    /**
     * Construct a FloatVolumeVector from an array of FloatVolume objects. The FloatVolume values are each expressed in their own
     * unit, but will be internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data FloatVolume[]; the data for the vector
     * @param displayUnit VolumeUnit; the display unit of the values when printing
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    ...
    /**
     * Construct a FloatVolumeVector from a list of Number objects or a list of FloatVolume objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatVolume objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatVolume objects, each FloatVolume has its
     ...
     * @param data List&lt;Float&gt; or List&lt;Volume&gt;; the data for the vector
     * @param displayUnit VolumeUnit; the display unit of the vector data, and the unit of the data points when the data is
     ...
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatVolume objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatVolume> */

    /**
     * Construct a FloatVolumeVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatVolume objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatVolume objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatVolume objects, each FloatVolume has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatVolume&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit VolumeUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size, final VolumeUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatVolume
     ...
     * assume that they are expressed using SI units. When the data consists of FloatVolume objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
