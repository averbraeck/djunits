...

{
    ...
    /**
     * Construct an FlowVolumeVector from a double[] object. The double values are expressed in the displayUnit, and will be
    ...
    /**
     * Construct an FlowVolumeVector from a double[] object with SI-unit values. Assume that the StorageType is DENSE since we
    ...
    /**
     * Construct an FlowVolumeVector from an array of FlowVolume objects. The FlowVolume values are each expressed in their own
     * unit, but will be internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data FlowVolume[]; the data for the vector
    ...
    /**
     * Construct an FlowVolumeVector from a list of Number objects or a list of FlowVolume objects. Note that the displayUnit
     * has a different meaning depending on whether the list contains Number objects (e.g., Double objects) or FlowVolume
     * objects. In case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are
     * expressed, as well as the unit in which they will be printed. In case the list contains FlowVolume objects, each
     * FlowVolume has its own unit, and the displayUnit is just used for printing. The values but will always be internally
     * stored as SI values or base values, and expressed using the display unit or base unit when printing.
     * @param data List&lt;Double&gt; or List&lt;FlowVolume&gt;; the data for the vector
    ...
    /**
     * Construct an FlowVolumeVector from a list of Number objects or a list of FlowVolume objects. When data contains numbers
     * such as Double, assume that they are expressed using SI units. When the data consists of FlowVolume objects, they each
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Double> or SortedMap<Integer, FlowVolume> */

    /**
     * Construct an FlowVolumeVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FlowVolume objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the
     * size of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a
     * different meaning depending on whether the map contains Number objects (e.g., Double objects) or FlowVolume objects. In
     * case the map contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as
     * well as the unit in which they will be printed. In case the map contains FlowVolume objects, each FlowVolume has its own
     ...
     * @param data SortedMap&lt;Integer, Double&gt; or SortedMap&lt;Integer, FlowVolume&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit FlowVolumeUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FlowVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size, final FlowVolumeUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FlowVolume
     ...
     * Double, assume that they are expressed using SI units. When the data consists of FlowVolume objects, they each have their
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FlowVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FlowVolumeVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
