...

{
    ...
    /**
     * Construct a FloatMassVector from a float[] object. The Float values are expressed in the displayUnit, and will be
    ...
    /**
     * Construct a FloatMassVector from a list of Number objects or a list of FloatMass objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatMass objects. In
    ...
    /**
     * Construct a FloatMassVector from a list of Number objects or a list of FloatMass objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatMass objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatMass> */

    /**
     * Construct a FloatMassVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatMass objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatMass&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit MassUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatMassVector(final SortedMap<Integer, ? extends Number> data, final int size, final MassUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatMass
     ...
     * assume that they are expressed using SI units. When the data consists of FloatMass objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatMassVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatMassVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
