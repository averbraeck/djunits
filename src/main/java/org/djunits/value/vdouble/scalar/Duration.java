...
public class Duration extends DoubleScalarRelWithAbs<TimeUnit, Time, DurationUnit, Duration>
{
    ...
    /**
     * Multiply two scalars that result in a scalar of type Duration.
     * @param scalar1 the first scalar
     * @param scalar2 the second scalar
     * @return the multiplication of both scalars as an instance of Duration
     */
    public static Duration multiply(final DoubleScalarRel<?, ?> scalar1, final DoubleScalarRel<?, ?> scalar2)
    {
        ...
        Throw.when(!scalar1.getDisplayUnit().getQuantity().getSiDimensions()
                ...
                IllegalArgumentException.class, "Multiplying %s by %s does not result in instance of type Duration",
                scalar1.toDisplayString(), scalar2.toDisplayString());
        return new Duration(scalar1.si * scalar2.si, DurationUnit.SI);
    }

    /**
     * Divide two scalars that result in a scalar of type Duration.
     * @param scalar1 the first scalar
     * @param scalar2 the second scalar
     * @return the division of scalar1 by scalar2 as an instance of Duration
     */
    public static Duration divide(final DoubleScalarRel<?, ?> scalar1, final DoubleScalarRel<?, ?> scalar2)
    {
        Throw.whenNull(scalar1, "scalar1 cannot be null");
        Throw.whenNull(scalar2, "scalar2 cannot be null");
        Throw.when(!scalar1.getDisplayUnit().getQuantity().getSiDimensions()
                .minus(scalar2.getDisplayUnit().getQuantity().getSiDimensions()).equals(DurationUnit.BASE.getSiDimensions()),
                IllegalArgumentException.class, "Dividing %s by %s does not result in an instance of type Duration",
                scalar1.toDisplayString(), scalar2.toDisplayString());
        return new Duration(scalar1.si / scalar2.si, DurationUnit.SI);
    }
