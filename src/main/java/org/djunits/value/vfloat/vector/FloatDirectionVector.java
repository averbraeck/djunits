...
public class FloatDirectionVector extends FloatVectorAbs<DirectionUnit, FloatDirection, FloatDirectionVector,
    AngleUnit, FloatAngle, FloatAngleVector>
{
    ...
    /**
     * Construct a FloatDirectionVector from a float[] object. The Float values are expressed in the displayUnit. Assume that the
    ...
    /**
     * Construct a FloatDirectionVector from an array of FloatDirection objects. The FloatDirection values are each expressed in their own
    ...
    /**
     * Construct a FloatDirectionVector from a list of Number objects or a list of FloatDirection objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatDirection objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatDirection objects, each FloatDirection has its
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatDirectionVector(final List<? extends Number> data, final DirectionUnit displayUnit, final StorageType storageType)
    {
    ...
    /**
     * Construct a FloatDirectionVector from a list of Number objects or a list of FloatDirection objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatDirection objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatDirection> */

    /**
     * Construct a FloatDirectionVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatDirection objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatDirection objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatDirection objects, each FloatDirection has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatDirection&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit DirectionUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatDirectionVector(final SortedMap<Integer, ? extends Number> data, final int size, final DirectionUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatDirection
     ...
     * assume that they are expressed using SI units. When the data consists of FloatDirection objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatDirectionVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatDirectionVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
