...
public class FloatPositionVector extends FloatVectorAbs<PositionUnit, FloatPosition, FloatPositionVector,
    LengthUnit, FloatLength, FloatLengthVector>
{
     ...
     * @param displayUnit PositionUnit; the display unit of the vector data
     */
    ...
    /**
     * Construct a FloatPositionVector from an array of FloatPosition objects. The FloatPosition values are each expressed in their own
     * unit, but will be internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data FloatPosition[]; the data for the vector
    ...
    /**
     * Construct a FloatPositionVector from a list of Number objects or a list of FloatPosition objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatPosition objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatPosition objects, each FloatPosition has its
     ...
     * @param data List&lt;Float&gt; or List&lt;Position&gt;; the data for the vector
    ...
    /**
     * Construct a FloatPositionVector from a list of Number objects or a list of FloatPosition objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatPosition objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatPosition> */

    /**
     * Construct a FloatPositionVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatPosition objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatPosition objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatPosition objects, each FloatPosition has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatPosition&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit PositionUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatPositionVector(final SortedMap<Integer, ? extends Number> data, final int size, final PositionUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatPosition
     ...
     * assume that they are expressed using SI units. When the data consists of FloatPosition objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatPositionVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatPositionVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
