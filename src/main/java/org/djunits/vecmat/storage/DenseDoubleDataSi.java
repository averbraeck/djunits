...
public class DenseDoubleDataSi implements DataGridSi<DenseDoubleDataSi>
{
     ...
     *             or columns is not positive
     */
    public static DenseDoubleDataSi ofSi(final double[] data, final int rows, final int cols)
    {
        return new DenseDoubleDataSi(data.clone(), rows, cols);
    }
     ...
     */
    public static <Q extends Quantity<Q>> DenseDoubleDataSi of(final double[] dataInUnit, final Unit<?, Q> unit, final int rows,
            final int cols)
    {
    ...
    public static DenseDoubleDataSi ofSi(final double[][] data)
    {
        Throw.whenNull(data, "data");
     ...
     */
    @SuppressWarnings("checkstyle:needbraces")
    public static <Q extends Quantity<Q>> DenseDoubleDataSi of(final double[][] dataInUnit, final Unit<?, Q> unit)
    {
        ...
        for (int r = 0; r < rows; r++)
        {
            Throw.when(dataInUnit[r].length != cols, IllegalArgumentException.class,
                    ...
                    cols);
            for (int c = 0; c < cols; c++)
                dataSi[r * cols + c] = unit.toBaseValue(dataInUnit[r][c]);
        }
        return new DenseDoubleDataSi(dataSi, rows, cols);
    }
     ...
     */
    @SuppressWarnings("checkstyle:needbraces")
    public static <Q extends Quantity<Q>> DenseDoubleDataSi of(final Q[][] data)
    {
        Throw.whenNull(data, "data");
        Throw.when(data.length == 0, IllegalArgumentException.class, "Number of rows in the data matrix = 0");
        int rows = data.length;
