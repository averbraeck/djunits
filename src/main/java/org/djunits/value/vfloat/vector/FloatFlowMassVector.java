...
public class FloatFlowMassVector extends FloatVectorRel<FlowMassUnit, FloatFlowMass, FloatFlowMassVector>

{
    ...
    /* CONSTRUCTORS WITH FloatFlowMass[] */

    /**
     * Construct a FloatFlowMassVector from an array of FloatFlowMass objects. The FloatFlowMass values are each expressed in their own
    ...
    /* CONSTRUCTORS WITH List<Float> or List<FlowMass> */

    /**
     * Construct a FloatFlowMassVector from a list of Number objects or a list of FloatFlowMass objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatFlowMass objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatFlowMass objects, each FloatFlowMass has its
    ...
    public FloatFlowMassVector(final List<? extends Number> data, final FlowMassUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(new float[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof FloatFlowMass ? FloatVectorData.instantiate(data, IdentityScale.SCALE, storageType)
    ...
    /**
     * Construct a FloatFlowMassVector from a list of Number objects or a list of FloatFlowMass objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatFlowMass objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatFlowMass> */

    /**
     * Construct a FloatFlowMassVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatFlowMass objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatFlowMass objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatFlowMass objects, each FloatFlowMass has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatFlowMass&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit FlowMassUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatFlowMassVector(final SortedMap<Integer, ? extends Number> data, final int size, final FlowMassUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatFlowMass
     ...
     * assume that they are expressed using SI units. When the data consists of FloatFlowMass objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatFlowMassVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatFlowMassVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
