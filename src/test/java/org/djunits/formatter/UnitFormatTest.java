...
public class UnitFormatTest
{
    ...
    public void testDisplayUnit()
    {
        ...
        String s2 = energy.toString(QuantityFormat.defaults().setDisplayUnit(Energy.Unit.kJ));
        assertTrue(s2.contains("1.2"));
        assertTrue(s2.endsWith(" kJ"));

        String s3 = energy.toString(QuantityFormat.defaults().setDisplayUnit("kJ"));
        assertTrue(s3.contains("1.2"));
        assertTrue(s3.endsWith(" kJ"));

        String s4 = energy.toString(QuantityFormat.defaults().setDisplayUnit("xyz"));
        assertTrue(s4.contains("1200"));
        assertTrue(s4.endsWith(" J"));

        String s5 = energy.toString(QuantityFormat.defaults().setDisplayUnit(Area.Unit.m2));
    ...
    public void testDisplayTextual()
    {
        ...
        String s1 = temp.toString(QuantityFormat.defaults().display());
        assertTrue(s1.contains("21"));
        assertTrue(s1.endsWith(" \u00B0C"));

        String s2 = temp.toString(QuantityFormat.defaults().textual());
        assertTrue(s2.contains("21"));
        assertTrue(s2.endsWith(" degC"));

        String s3 = temp.toString(QuantityFormat.defaults().textual(false));
        assertTrue(s3.contains("21"));
        assertTrue(s3.endsWith(" \u00B0C"));

        String s4 = temp.toString(QuantityFormat.defaults().textual(true));
    ...
    public void testSIUnits()
    {
        ...
        String s1 = energy.toString(QuantityFormat.defaults().siUnits());
        assertTrue(s1.contains("1200.345"));
        assertTrue(s1.endsWith(" kgm2/s2"));

        String s2 = energy.toString(QuantityFormat.defaults().siUnits().divider(true));
        assertTrue(s2.contains("1200.345"));
        assertTrue(s2.endsWith(" kgm2/s2"));

        String s3 = energy.toString(QuantityFormat.defaults().siUnits().divider(false));
        assertTrue(s3.contains("1200.345"));
        assertTrue(s3.endsWith(" kgm2s-2"));

        String s4 = energy.toString(QuantityFormat.defaults().siUnits().dotSeparator("."));
        assertTrue(s4.contains("1200.345"));
        assertTrue(s4.endsWith(" kg.m2/s2"));

        String s5 = energy.toString(QuantityFormat.defaults().siUnits().powerPrefix("^"));
        assertTrue(s5.contains("1200.345"));
        assertTrue(s5.endsWith(" kgm^2/s^2"));

        String s6 = energy.toString(QuantityFormat.defaults().siUnits().powerPrefix("<sup>").powerPostfix("</sup>"));
        assertTrue(s6.contains("1200.345"));
        assertTrue(s6.endsWith(" kgm<sup>2</sup>/s<sup>2</sup>"));

        String s7 = energy.toString(QuantityFormat.defaults().siUnits().powerPrefix("<sup>").powerPostfix("</sup>"));
