...
public class Angle extends DoubleScalarRelWithAbs<DirectionUnit, Direction, AngleUnit, Angle>
{
    ...
    /**
     * Construct Angle scalar with a unit.
    ...
    /**
     * Construct Angle scalar based on an SI value.
     ...
     * @return a Angle at the given ratio between 0 and 1
     */
    ...
    public static Angle of(final double value, final String unitString)
    {
        ...
        AngleUnit unit = AngleUnit.BASE.getUnitByAbbreviation(unitString);
        Throw.when(unit == null, IllegalArgumentException.class, "Error parsing Angle with unit %s", unitString);
        return new Angle(value, unit);
    }
    ...
    /**
     * Multiply two scalars that result in a scalar of type Angle.
     * @param scalar1 the first scalar
     * @param scalar2 the second scalar
     * @return the multiplication of both scalars as an instance of Angle
     */
    public static Angle multiply(final DoubleScalarRel<?, ?> scalar1, final DoubleScalarRel<?, ?> scalar2)
    {
        ...
        Throw.when(!scalar1.getDisplayUnit().getQuantity().getSiDimensions()
                ...
                IllegalArgumentException.class, "Multiplying %s by %s does not result in instance of type Angle",
                scalar1.toDisplayString(), scalar2.toDisplayString());
        return new Angle(scalar1.si * scalar2.si, AngleUnit.SI);
    }

    /**
     * Divide two scalars that result in a scalar of type Angle.
     * @param scalar1 the first scalar
     * @param scalar2 the second scalar
     * @return the division of scalar1 by scalar2 as an instance of Angle
     */
    public static Angle divide(final DoubleScalarRel<?, ?> scalar1, final DoubleScalarRel<?, ?> scalar2)
    {
        Throw.whenNull(scalar1, "scalar1 cannot be null");
        Throw.whenNull(scalar2, "scalar2 cannot be null");
        Throw.when(!scalar1.getDisplayUnit().getQuantity().getSiDimensions()
                .minus(scalar2.getDisplayUnit().getQuantity().getSiDimensions()).equals(AngleUnit.BASE.getSiDimensions()),
                IllegalArgumentException.class, "Dividing %s by %s does not result in an instance of type Angle",
                scalar1.toDisplayString(), scalar2.toDisplayString());
        return new Angle(scalar1.si / scalar2.si, AngleUnit.SI);
    }
