...
- `adjugate()` returns the adjugate (classical adjoint) matrix for this matrix, often denoted as $adj(M)$. When the unit of the original matrix is $U$, the unit of $adj(M)$ is $U^{(n-1)}$. The adjugate of a square matrix \(A\) is the matrix $\(\operatorname{adj}(A)\) satisfying \(A \cdot \operatorname{adj}(A) = \det(A)\,I\)$.
- `normFrobenius()` returns the Frobenius norm of the matrix, which is equal to $\sqrt(\operatorname{trace}(A^*\cdot A))$. It results in a quantity with the same unit as the original matrix. See [Frobenius norm on Wikipedia](https://en.wikipedia.org/wiki/Matrix_norm#Frobenius_norm) for more information.
