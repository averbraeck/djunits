...
* **org.djunits.util** provides a number of classes to carry out calculations on arrays of values (`ArrayMath`), on matrices (`MatrixMath`), and a number of helper functions for, e.g., min, max, and sum of an unlimited numbr of values (`Math2`). A file with physical constants (`Constants`) is also found here.
