...
public class SolidAngleVector extends DoubleVectorRel<SolidAngleUnit, SolidAngle, SolidAngleVector>

{
    ...
    /**
     * Construct an SolidAngleVector from a double[] object. The double values are expressed in the displayUnit, and will be printed
    ...
    /**
     * Construct an SolidAngleVector from a double[] object with SI-unit values. Assume that the StorageType is DENSE since we offer
    ...
    /**
     * Construct an SolidAngleVector from an array of SolidAngle objects. The SolidAngle values are each expressed in their own unit, but will be
    ...
    /* CONSTRUCTORS WITH List<Double> or List<SolidAngle> */

    /**
     * Construct an SolidAngleVector from a list of Number objects or a list of SolidAngle objects. Note that the displayUnit has a
     * different meaning depending on whether the list contains Number objects (e.g., Double objects) or SolidAngle objects. In case
     * the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed, as
     * well as the unit in which they will be printed. In case the list contains SolidAngle objects, each SolidAngle has its own unit, and
     ...
     * @param data the data for the vector as a List&lt;Double&gt; or List&lt;SolidAngle&gt;
    ...
    public SolidAngleVector(final List<? extends Number> data, final SolidAngleUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(new double[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof SolidAngle ? DoubleVectorData.instantiate(data, IdentityScale.SCALE, storageType)
    ...
    /**
     * Construct an SolidAngleVector from a list of Number objects or a list of SolidAngle objects. When data contains numbers such as
     * Double, assume that they are expressed using SI units. When the data consists of SolidAngle objects, they each have their own
    ...
    /* CONSTRUCTORS WITH Map<Integer, Double> or Map<Integer, SolidAngle> */

    /**
     * Construct an SolidAngleVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to of
     * SolidAngle objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size of the
     * vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different meaning
     * depending on whether the map contains Number objects (e.g., Double objects) or SolidAngle objects. In case the map contains
     * Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the unit in
     * which they will be printed. In case the map contains SolidAngle objects, each SolidAngle has its own unit, and the displayUnit is
     ...
     * @param data the data for the vector as a Map&lt;Integer, Double&gt; or Map&lt;Integer, SolidAngle&gt;
            ...
            final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.values().iterator().next() instanceof SolidAngle
     ...
     * that they are expressed using SI units. When the data consists of SolidAngle objects, they each have their own unit, but will
