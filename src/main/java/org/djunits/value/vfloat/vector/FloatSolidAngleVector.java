...

{
    ...
    /**
     * Construct a FloatSolidAngleVector from a float[] object. The Float values are expressed in the displayUnit. Assume that the
    ...
    /**
     * Construct a FloatSolidAngleVector from a float[] object with SI-unit values. Assume that the StorageType is DENSE since we
    ...
    /**
     * Construct a FloatSolidAngleVector from an array of FloatSolidAngle objects. The FloatSolidAngle values are each expressed in their own
    ...
    /**
     * Construct a FloatSolidAngleVector from a list of Number objects or a list of FloatSolidAngle objects. Note that the displayUnit has
     * a different meaning depending on whether the list contains Number objects (e.g., Float objects) or FloatSolidAngle objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains FloatSolidAngle objects, each FloatSolidAngle has its
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatSolidAngleVector(final List<? extends Number> data, final SolidAngleUnit displayUnit, final StorageType storageType)
    {
    ...
    /**
     * Construct a FloatSolidAngleVector from a list of Number objects or a list of FloatSolidAngle objects. When data contains numbers
     * such as Float, assume that they are expressed using SI units. When the data consists of FloatSolidAngle objects, they each have
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Float> or SortedMap<Integer, FloatSolidAngle> */

    /**
     * Construct a FloatSolidAngleVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of FloatSolidAngle objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size
     * of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Float objects) or FloatSolidAngle objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains FloatSolidAngle objects, each FloatSolidAngle has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Float&gt; or SortedMap&lt;Integer, FloatSolidAngle&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit SolidAngleUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Float&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatSolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size, final SolidAngleUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? FloatVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof FloatSolidAngle
     ...
     * assume that they are expressed using SI units. When the data consists of FloatSolidAngle objects, they each have their own
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public FloatSolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public FloatSolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
