...
public class PressureVector extends DoubleVectorRel<PressureUnit, Pressure, PressureVector>

{
    ...
    /**
     * Construct an PressureVector from a double[] object. The double values are expressed in the displayUnit, and will be
     * printed using the displayUnit.
     * @param data double[]; the data for the vector, expressed in the displayUnit
     * @param displayUnit PressureUnit; the unit of the values in the data array, and display unit when printing
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final double[] data, final PressureUnit displayUnit, final StorageType storageType)
    {
        this(DoubleVectorData.instantiate(data, displayUnit.getScale(), storageType), displayUnit);
    }

    /**
     * Construct an PressureVector from a double[] object. The double values are expressed in the displayUnit. Assume that the
     * StorageType is DENSE since we offer the data as an array.
     * @param data double[]; the data for the vector
     * @param displayUnit PressureUnit; the unit of the values in the data array, and display unit when printing
     */
    public PressureVector(final double[] data, final PressureUnit displayUnit)
    {
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final double[] data, final StorageType storageType)
    {
        this(data, PressureUnit.SI, storageType);
    }

    /**
     * Construct an PressureVector from a double[] object with SI-unit values. Assume that the StorageType is DENSE since we
     * offer the data as an array.
     * @param data double[]; the data for the vector, in SI units
     */
    public PressureVector(final double[] data)
    {
    ...
    /**
     * Construct an PressureVector from an array of Pressure objects. The Pressure values are each expressed in their own unit,
     * but will be internally stored as SI values, all expressed in the displayUnit when printing.
     * @param data Pressure[]; the data for the vector
     * @param displayUnit PressureUnit; the display unit of the values when printing
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final Pressure[] data, final PressureUnit displayUnit, final StorageType storageType)
    {
     ...
     * @param displayUnit PressureUnit; the display unit of the values when printing
     */
    public PressureVector(final Pressure[] data, final PressureUnit displayUnit)
    {
        this(data, displayUnit, StorageType.DENSE);
    }
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final Pressure[] data, final StorageType storageType)
    {
        this(data, PressureUnit.SI, storageType);
    }
     ...
     * @param data Pressure[]; the data for the vector
     */
    public PressureVector(final Pressure[] data)
    {
        this(data, StorageType.DENSE);
    }

    /* CONSTRUCTORS WITH List<Double> or List<Pressure> */

    /**
     * Construct an PressureVector from a list of Number objects or a list of Pressure objects. Note that the displayUnit has a
     * different meaning depending on whether the list contains Number objects (e.g., Double objects) or Pressure objects. In
     * case the list contains Number objects, the displayUnit indicates the unit in which the values in the list are expressed,
     * as well as the unit in which they will be printed. In case the list contains Pressure objects, each Pressure has its own
     ...
     * @param data List&lt;Double&gt; or List&lt;Pressure&gt;; the data for the vector
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final List<? extends Number> data, final PressureUnit displayUnit, final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(new double[] {}, IdentityScale.SCALE, storageType)
                : data.get(0) instanceof Pressure ? DoubleVectorData.instantiate(data, IdentityScale.SCALE, storageType)
     ...
     *            expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     */
    public PressureVector(final List<? extends Number> data, final PressureUnit displayUnit)
    {
        this(data, displayUnit, StorageType.DENSE);
    }

    /**
     * Construct an PressureVector from a list of Number objects or a list of Pressure objects. When data contains numbers such
     * as Double, assume that they are expressed using SI units. When the data consists of Pressure objects, they each have
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final List<? extends Number> data, final StorageType storageType)
    {
        this(data, PressureUnit.SI, storageType);
    }
     ...
     * @param data List&lt;Double&gt; or List&lt;Pressure&gt;; the data for the vector
     */
    public PressureVector(final List<? extends Number> data)
    {
        this(data, StorageType.DENSE);
    }

    /* CONSTRUCTORS WITH SortedMap<Integer, Double> or SortedMap<Integer, Pressure> */

    /**
     * Construct an PressureVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to of
     * Pressure objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the size of
     * the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a different
     * meaning depending on whether the map contains Number objects (e.g., Double objects) or Pressure objects. In case the map
     * contains Number objects, the displayUnit indicates the unit in which the values in the map are expressed, as well as the
     * unit in which they will be printed. In case the map contains Pressure objects, each Pressure has its own unit, and the
     ...
     * @param data SortedMap&lt;Integer, Double&gt; or SortedMap&lt;Integer, Pressure&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit PressureUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final SortedMap<Integer, ? extends Number> data, final int size, final PressureUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof Pressure
     ...
     * assume that they are expressed using SI units. When the data consists of Pressure objects, they each have their own unit,
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public PressureVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public PressureVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
