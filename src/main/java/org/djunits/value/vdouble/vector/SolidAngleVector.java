...

{
    ...
    /**
     * Construct an SolidAngleVector from a list of Number objects or a list of SolidAngle objects. Note that the displayUnit
     * has a different meaning depending on whether the list contains Number objects (e.g., Double objects) or SolidAngle
    ...
    /**
     * Construct an SolidAngleVector from a list of Number objects or a list of SolidAngle objects. When data contains numbers
    ...
    /* CONSTRUCTORS WITH SortedMap<Integer, Double> or SortedMap<Integer, SolidAngle> */

    /**
     * Construct an SolidAngleVector from a (sparse) map of index values to Number objects or a (sparse) map of index values to
     * of SolidAngle objects. Using index values is particularly useful for sparse vectors. The size parameter indicates the
     * size of the vector, since the largest index does not have to be part of the map. Note that the displayUnit has a
     * different meaning depending on whether the map contains Number objects (e.g., Double objects) or SolidAngle objects. In
     ...
     * @param data SortedMap&lt;Integer, Double&gt; or SortedMap&lt;Integer, SolidAngle&gt;; the data for the vector
     * @param size int; the size off the vector, i.e., the highest index
     * @param displayUnit SolidAngleUnit; the display unit of the vector data, and the unit of the data points when the data is
     *            expressed as List&lt;Double&gt; or List&lt;Number&gt; in general
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public SolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size, final SolidAngleUnit displayUnit,
            final StorageType storageType)
    {
        this(data.size() == 0 ? DoubleVectorData.instantiate(data, size, IdentityScale.SCALE, storageType)
                : data.get(data.firstKey()) instanceof SolidAngle
     ...
     * @param storageType StorageType; the StorageType (SPARSE or DENSE) to use for constructing the Vector
     */
    public SolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size, final StorageType storageType)
    {
     ...
     * @param size int; the size off the vector, i.e., the highest index
     */
    public SolidAngleVector(final SortedMap<Integer, ? extends Number> data, final int size)
    {
