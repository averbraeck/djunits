...
- logical operators such as `lt0`, `le0`, `eq0`, `ne0` to check whether one quantity is, e.g., bigger or smaller than zero. These methods are not available for absolute quantities.
- `String toString()` with several variants for formatting the unit, so it can be expressed, e.g., as `kgm/s2` or `kg.m.s-2` or as a HTML string `kg.m.s<sup>-2</sup>`, where the latter displays as kg.m.s<sup>-2</sup>.
...
- `Q add(Q increment)` returns a new quantity where the SI value has been increased by the SI value from `increment`. The display unit of the resulting value is the same as the display unit of the quantity `p` when `p.add(q)` is called.
- `Q subtract(Q decrement)` returns a quantity where the SI value from `decrement` is subtracted from the current quantity. The display unit of the resulting value is the same as the display unit of the quantity `p` when `p.subtract(q)` is called.
...
- `SIQuantity multiply(Quantity quantity)` yields the product of a quantity with another quantity.
- `SIQuantity divide(Quantity quantity)` yields the division of a quantity by another quantity.
...
Quantities have a `toString()` method that returns a String representation of the quantity using a (localized) floating point representation of the value in the display unit, followed by a space and the (localized) unit. Formatting options are available through the `format` method:
